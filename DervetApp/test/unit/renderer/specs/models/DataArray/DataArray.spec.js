...
describe('DataArray model', () => {
  ...
  it('should return invalidCheckRowsCount properly', () => {
  ...
  it('should return invalidCheckRowSize properly', () => {
    let actual = fakeData.invalidCheckRowSize(1);
    let expected = { errorMsg: '<b>2 Invalid Rows:</b> each row must have 1 value : [5,6]' };
    expect(actual).to.eql(expected);
  });

  it('should return invalidCheckSingleValueNumeric properly', () => {
    let actual = fakeData.invalidCheckSingleValueNumeric();
    let expected = { errorMsg: '<b>1 Invalid Rows:</b> each value must be numeric : [3]' };
    expect(actual).to.eql(expected);
  });
