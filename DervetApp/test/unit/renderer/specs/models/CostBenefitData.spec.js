...
describe('CostBenefitData model', () => {
  ...
  it('(1A) should load data and column headers', () => {
    ...
    const expectedColumnHeaders = ['', 'Cost ($)', 'Benefit ($)'];
  ...
  it('(1C) should create the correct trace objects', () => {
    ...
    while (i < traces.length) {
      expect(traces[i].systemName).to.eql(costBenefitTraces[i].systemName);

      const actualProperties = Object.getOwnPropertyNames(traces[i]);
      const expectedProperties = Object.getOwnPropertyNames(costBenefitTraces[i]);
